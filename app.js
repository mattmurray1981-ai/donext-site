...
(function () {
  ...
  // ============================================================
  // DATED CARDIFF REPORT — local, sourced data rendered by day
  // ============================================================
  var _reportLoaded = false;
  var _selectedReportDate = null;
  var _reportEventsById = {};
  var _featuredByDate = {};
  ...
  function londonDateKey(date) {
    var parts = londonDateParts(date);
  ...
  function buildFeaturedSchedule(report) {
    var used = {};
    var schedule = {};

    report.days.forEach(function (day) {
      var candidates = [day.featuredEventId].concat(day.eventIds || []);
      var chosen = null;

      candidates.some(function (eventId) {
        var event = _reportEventsById[eventId];
        if (event && event.dates.indexOf(day.date) !== -1 && !used[eventId]) {
          chosen = eventId;
          return true;
        }
        return false;
  ...
  function chooseReportDate(report) {
    var todayKey = londonDateKey(new Date());
    var dates = report.days.map(function (day) { return day.date; });
    if (dates.indexOf(todayKey) !== -1) return todayKey;
    if (todayKey < dates[0]) return dates[0];
    return dates[dates.length - 1];
  }

  function updateFreshness(report) {
    var badge = document.getElementById('cardiff-report-freshness');
    if (!badge) return;

    var checked = new Date(report.checkedAt);
    var nowParts = londonDateParts(new Date());
    var checkedParts = londonDateParts(checked);
    var difference = calendarDayNumber(nowParts) - calendarDayNumber(checkedParts);
    var time = checked.toLocaleTimeString('en-GB', {
      timeZone: 'Europe/London',
      hour: '2-digit',
      minute: '2-digit'
    });
    ...
    if (difference === 0) {
      badge.textContent = 'Checked today at ' + time;
    } else if (difference === 1) {
      badge.textContent = 'Checked yesterday at ' + time;
    } else {
      badge.textContent = 'Checked ' + checked.toLocaleDateString('en-GB', {
        timeZone: 'Europe/London',
        day: 'numeric',
        month: 'long',
        year: 'numeric'
  ...
  function renderReportTabs(report) {
    var tabs = document.getElementById('cardiff-report-days');
    if (!tabs) return;

    tabs.innerHTML = report.days.map(function (day) {
      var active = day.date === _selectedReportDate;
      return '<button class="bucket-tab report-date-tab' + (active ? ' active' : '') + '"' +
        ' type="button" role="tab" aria-selected="' + (active ? 'true' : 'false') + '"' +
        ' aria-controls="cardiff-report-content" data-report-date="' + escapeHtml(day.date) + '">' +
        escapeHtml(day.tabLabel) + '</button>';
    }).join('');

    tabs.querySelectorAll('[data-report-date]').forEach(function (tab) {
      tab.addEventListener('click', function () {
        renderReportDay(this.getAttribute('data-report-date'));
      });
    });
  }

  function renderWeather(report, selectedDay) {
    var weather = document.getElementById('cardiff-report-weather');
    if (!weather) return;

    var checked = new Date(report.weatherCheckedAt);
    var checkedLabel = checked.toLocaleDateString('en-GB', {
      timeZone: 'Europe/London',
      day: 'numeric',
      month: 'long'
    ...
    var daysHtml = report.days.map(function (day) {
      var active = day.date === selectedDay.date;
      return '<div class="weather-day' + (active ? ' weather-day--active' : '') + '" role="listitem"' +
        ' aria-label="' + escapeHtml(day.label + ': ' + day.weather.summary + ', high ' + day.weather.high) + '">' +
        '<div class="weather-day__name">' + escapeHtml(day.tabLabel) + '</div>' +
        '<div class="weather-day__icon" role="img" aria-label="' + escapeHtml(day.weather.summary) + '">' + escapeHtml(day.weather.icon) + '</div>' +
        '<div class="weather-day__temp">' + escapeHtml(day.weather.high) + '</div>' +
        '<div class="weather-day__low">' + escapeHtml(day.weather.summary) + '</div>' +
      '</div>';
    }).join('');

    weather.setAttribute('aria-label', 'Cardiff forecast snapshot for ' + report.rangeLabel);
    weather.innerHTML = '<div class="weather-bar__title">Forecast snapshot checked ' + escapeHtml(checkedLabel) +
      ' &middot; <a class="report-inline-link" href="' + escapeHtml(report.weatherSource.url) + '" target="_blank" rel="noopener noreferrer">' +
      escapeHtml(report.weatherSource.name) + '</a></div>' +
      '<div class="weather-days weather-days--report" role="list">' + daysHtml + '</div>' +
      '<p class="weather-advice"><strong>' + escapeHtml(selectedDay.label) + ':</strong> ' + escapeHtml(selectedDay.weather.advice) + '</p>';
  }

  function renderSources(event) {
    return (event.sources || []).map(function (source, index) {
      var label = event.sources.length > 1 ? 'Source ' + (index + 1) : 'Source';
      return '<a href="' + escapeHtml(source.url) + '" class="event-card__link" target="_blank" rel="noopener noreferrer"' +
        ' aria-label="' + escapeHtml(label + ': ' + source.name + ' (opens in a new tab)') + '">' +
  ...
  function renderEventCard(event, selectedDay) {
    var isFree = event.cost.toLowerCase().indexOf('free') === 0;
    var confidence = event.confidence === 'medium'
      ? '<span class="event-card__status">Check before travel</span>'
      : '';
    var status = event.statusNote
      ? '<p class="event-card__status-note">' + escapeHtml(event.statusNote) + '</p>'
      : '';

    return '<article class="event-card" role="listitem" data-event-id="' + escapeHtml(event.id) + '">' +
      '<div class="event-card__main">' +
        '<div class="event-card__name">' + escapeHtml(event.title) + '</div>' +
        '<div class="event-card__meta">' +
          '<span class="event-card__meta-item"><svg width="12" height="12" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" aria-hidden="true"><rect x="3" y="4" width="18" height="18" rx="2"/><line x1="16" y1="2" x2="16" y2="6"/><line x1="8" y1="2" x2="8" y2="6"/><line x1="3" y1="10" x2="21" y2="10"/></svg>' + escapeHtml(selectedDay.label + ', ' + event.time) + '</span>' +
          '<span class="event-card__meta-item"><svg width="12" height="12" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" aria-hidden="true"><path d="M21 10c0 7-9 13-9 13s-9-6-9-13a9 9 0 0 1 18 0z"/><circle cx="12" cy="10" r="3"/></svg>' + escapeHtml(event.location) + '</span>' +
        '</div>' +
        '<p class="event-card__desc">' + escapeHtml(event.description) + '</p>' +
        status +
      '</div>' +
      '<div class="event-card__side">' +
        '<div class="event-card__cost' + (isFree ? ' event-card__cost--free' : '') + '">' + escapeHtml(event.cost) + '</div>' +
        (event.ages ? '<div class="event-card__age">' + escapeHtml(event.ages) + '</div>' : '') +
        confidence +
        renderSources(event) +
      '</div>' +
    '</article>';
  ...
  function renderNotices(selectedDay) {
    var container = document.getElementById('cardiff-report-notices');
    if (!container) return;
    if (!selectedDay.notices || !selectedDay.notices.length) {
      container.innerHTML = '';
      return;
    }

    container.innerHTML = '<div class="report-notices" aria-label="Closures and important notes">' +
      selectedDay.notices.map(function (notice) {
        return '<div class="report-notice"><div><strong>' + escapeHtml(notice.title) + '</strong><p>' + escapeHtml(notice.detail) + '</p></div>' +
          '<a class="event-card__link" href="' + escapeHtml(notice.sourceUrl) + '" target="_blank" rel="noopener noreferrer">' +
          escapeHtml(notice.sourceName) + '<svg width="10" height="10" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.5" aria-hidden="true"><path d="M7 17L17 7M7 7h10v10"/></svg></a></div>';
      }).join('') + '</div>';
  }

  function renderFeatured(selectedDay) {
    var container = document.getElementById('cardiff-report-feature');
    if (!container) return;
    var event = _reportEventsById[_featuredByDate[selectedDay.date]];
    if (!event) {
      container.innerHTML = '';
      return;
    }

    container.innerHTML = '<div class="events-section events-section--featured" aria-labelledby="featured-event-heading">' +
      '<div class="events-section-header"><h2 class="events-section-title" id="featured-event-heading">Featured pick</h2>' +
      '<span class="events-section-badge badge--gem">A different pick each day</span></div>' +
      '<div class="event-list" role="list">' + renderEventCard(event, selectedDay) + '</div></div>';
  }

  function renderEventGroups(selectedDay) {
    var container = document.getElementById('cardiff-report-events');
    if (!container) return;
    var featuredId = _featuredByDate[selectedDay.date];
    var dayEvents = selectedDay.eventIds.map(function (eventId) {
      return _reportEventsById[eventId];
    }).filter(function (event) {
      return event && event.id !== featuredId && event.dates.indexOf(selectedDay.date) !== -1;
    });

    var groups = [
      { kind: 'event', title: 'More sourced events', badge: 'Date-specific and weekly listings', badgeClass: 'badge--gem' },
      { kind: 'anchor', title: 'Reliable anchors', badge: 'Opening dates checked', badgeClass: 'badge--anchor' }
    ];

    container.innerHTML = groups.map(function (group, groupIndex) {
      var events = dayEvents.filter(function (event) { return event.kind === group.kind; });
      if (!events.length) return '';
      var headingId = 'report-group-' + groupIndex;
      return '<div class="events-section" aria-labelledby="' + headingId + '">' +
        '<div class="events-section-header"><h2 class="events-section-title" id="' + headingId + '">' + escapeHtml(group.title) + '</h2>' +
        '<span class="events-section-badge ' + group.badgeClass + '">' + escapeHtml(group.badge) + '</span></div>' +
        '<div class="event-list" role="list">' + events.map(function (event) { return renderEventCard(event, selectedDay); }).join('') + '</div>' +
      '</div>';
    }).join('');
  }

  function renderReportDay(date) {
    var report = window.DONEXT_CARDIFF_REPORT;
    if (!report) return;
    var selectedDay = report.days.find(function (day) { return day.date === date; });
    if (!selectedDay) return;

    _selectedReportDate = date;
    if (window._donextReportState) window._donextReportState.selectedDate = date;
    renderReportTabs(report);
    renderWeather(report, selectedDay);
    renderNotices(selectedDay);
    renderFeatured(selectedDay);
    renderEventGroups(selectedDay);

    var heading = document.getElementById('cardiff-report-heading');
    if (heading) heading.textContent = selectedDay.label + ': family picks';
  }

  function loadDailyReport() {
    var report = window.DONEXT_CARDIFF_REPORT;
    if (!report || !report.days || !report.events) {
      var content = document.getElementById('cardiff-report-content');
      if (content) content.innerHTML = '<p class="report-error">The dated Cardiff report could not be loaded.</p>';
      return;
    }

    if (!_reportLoaded) {
      report.events.forEach(function (event) { _reportEventsById[event.id] = event; });
      _featuredByDate = buildFeaturedSchedule(report);
      _selectedReportDate = chooseReportDate(report);
      _reportLoaded = true;
      updateFreshness(report);
      window._donextReportState = {
        selectedDate: _selectedReportDate,
        featuredByDate: _featuredByDate
      };
    }

    renderReportDay(_selectedReportDate);
  }

  window._donextLoadReport = loadDailyReport;
  window._donextRenderReportDate = renderReportDay;
