...
export function brandFiles(source, avatar, share) {
  ...
  for (const file of htmlPaths) {
    ...
    html = html.replace('</head>', `  <link rel="icon" href="${avatarPath}" type="image/png" sizes="${av.width}x${av.height}">\n  <link rel="apple-touch-icon" href="${avatarPath}" sizes="${av.width}x${av.height}">\n</head>`);
...
if (process.argv[2]) {
  ...
  if (await fs.access(path.join(input, 'weekend-brief.html')).then(() => true).catch(() => false)) source.set('/weekend-brief.html', await fs.readFile(path.join(input, 'weekend-brief.html')));
