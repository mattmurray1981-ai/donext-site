...
All three direct destinations remain disabled in `accounts.json`; **no successful direct Meta delivery is recorded**. The queue is not necessarily empty: an entry's presence or `ready` status does not establish usable access or current research. Bristol Page `1278713381995669`, Cardiff Page `1003042559562888` and Birmingham Page `1264079476796047` are pinned in the account configuration. These identity pins are not proof that stored credentials or publishing permissions work. No credentials or subscriber records belong in this public repository.
...
Failed checks emit a structured `account-check` JSON line containing a fixed reason: `missing_token`, `meta_http_error`, `page_id_mismatch`, `instagram_account_missing`, `instagram_username_mismatch`, `network_error`, `malformed_response` or `unexpected_error`. A Meta HTTP failure can additionally include only numeric `httpStatus`, `metaCode` and `metaSubcode` fields. Raw provider messages and response bodies are never logged. An identity-check failure alone does **not** prove that a token is invalid: first distinguish missing credentials, the wrong token type, denied permissions, an unexpected Page/linkage, and a transient request failure. A copied Graph API Explorer **user** token is not the matching **Page** token required below. Missing Instagram linkage means it was not visible to that credential; it is not proof that no linkage exists in Meta's account settings.
