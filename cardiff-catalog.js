...
(function (global) {
  ...
  var CITIES = Object.freeze({
    ...
    birmingham: Object.freeze({ id: 'birmingham', name: 'Birmingham', catalogUrl: '/data/birmingham-today.json' })
  });
  ...
  function renderFreshness(data, now) {
    ...
    setText('catalog-freshness', stamp ? 'Shortlist updated ' + stamp + ' (UK)' : 'Update time unavailable');
    var banner = document.getElementById('stale-banner');
    if (!banner) return;
    var hours = (now - Date.parse(data.sourceCheckedAt || data.updatedAt)) / 3600000;
  ...
  function renderPickCard(pick, opts) {
    ...
    return '<article class="pick' + (opts.hero ? ' pick--hero' : '') + '" id="pick-' + escapeHtml(pick.id) + '" data-event-id="' + escapeHtml(pick.id) + '">' +
      ...
      (pick.location ? '<a class="pick__directions" href="https://www.google.com/maps/search/?api=1&amp;query=' + encodeURIComponent(pick.location) + '" target="_blank" rel="noopener noreferrer">Get directions ↗</a>' : '') +
